...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int row = 0; row < width(); row++) {
            for (int col = 0; col < height(); col++) {
                _solnNumToPlace[solution[row][col]] = Place.pl(row, col);
            }
        }
        int exist = 1;
        while (exist <= last) {
            if (solnNumToPlace(exist) == null) {
                throw badArgs("not all sequenceNums are instantiated");
            }
            exist++;
        }
        _board = new Sq[width()][height()];
        for (int i = 1; i < _solnNumToPlace.length; i++) {
            int seqNum = 1; int group = -1; int dir = 0;
            Place p = solnNumToPlace(i);
            if (i == last) {
                seqNum = i; group = 0;
            } else if (i == 1) {
                group = 0; dir = arrowDirection(p.x, p.y);
            } else {
                seqNum = 0; dir = arrowDirection(p.x, p.y);
            }
            _board[p.x][p.y] = new Sq(p.x, p.y, seqNum, group == 0, dir, group);
        }
        for (Sq[] col: _board) {
            for (Sq sq : col) {
                _allSquares.add(sq);
        ...
        for (Sq[] col: _board) {
            for (Sq square: col) {
                for (Place succ: square.successors()) {
                    Sq squa = get(succ.x, succ.y);
                    squa._predecessors.add(square.pl);
                }
            }
        }
        _unconnected = last - 1;
    ...
    Model(Model model) {
        ...
        for (int row = 0; row < _width; row++) {
            for (int col = 0; col < _height; col++) {
                Model.Sq origin = model._board[row][col];
                int sN = origin.sequenceNum(); int dir = origin.direction();
                int gp = origin.group(); boolean fix = origin.hasFixedNum();
                _board[row][col] = new Sq(origin.x, origin.y, sN, fix, dir, gp);
                _allSquares.add(_board[row][col]);
            }
        }
        for (Sq[] col: _board) {
            for (Sq square: col) {
                PlaceList x = _allSuccessors[square.x][square.y][square._dir];
                square._successors = x;
        ...
        for (Sq[] col: _board) {
            for (Sq square: col) {
                for (Place succ: square.successors()) {
                    Sq squa = _board[succ.x][succ.y];
                    squa._predecessors.add(square.pl);
                }
                Sq succ = model.get(square.x, square.y).successor();
                Sq pred = model.get(square.x, square.y).predecessor();
                if (succ != null) {
                    square._successor = _board[succ.x][succ.y];
                }
                if (pred != null) {
                    square._predecessor = _board[pred.x][pred.y];
                    square._head = pred.head();
                }
            }
        }
    }
     ...
     *  unconnected and are separated by a queen move.  Returns true if
     *  any changes were made. */
    boolean autoconnect() {
        boolean edited = false;
        for (Sq square: _allSquares) {
            for (Sq s1: _allSquares) {
                if (square.sequenceNum() + 1 == s1.sequenceNum()) {
                    square.connect(s1);
                    edited = true;
                }
            }
    ...
    void solve() {
        for (int row = 0; row < width(); row++) {
            for (int col = 0; col < height(); col++) {
                _board[row][col]._sequenceNum = solution()[row][col];
            }
        }
        autoconnect();
        _unconnected = 0;
    ...
    private int arrowDirection(int x, int y) {
        ...
        int dir = 0;
        for (int xx = 0; xx < _solution.length; xx++) {
            for (int yy = 0; yy < _solution[0].length; yy++) {
                if (_solution[xx][yy] == seq0 + 1) {
                    dir = signpost.Place.dirOf(x, y, xx, yy);
                }
            }
        }
        return dir;
    ...
    final class Sq {
         ...
         *  if none initially assigned), which is fixed if FIXED. */
        ...
        /** Return true if this square's sequence number is fixed. */
        boolean hasFixedNum() {
            return _hasFixedNum;
        }
        ...
        /** Returns true if this square may be connected to square S1, that is:
                ...
                if (0 != tdir) {
                    if (s1.predecessor() == null && this.successor() == null) {
                        ...
                        if (s1sN != 1 && this.sequenceNum() != size()) {
                            ...
                            boolean a = s1sN == 0;
                            boolean b = this.sequenceNum() == 0;
                            boolean d = this.group() == -1 || s1.group() == -1;
                            boolean c = this.group() != s1.group();
                            if ((a || b) && (d || c)) {
                                return true;
                            }
                        }
         ...
         *  nothing. Returns true if this square and S1 were connectable.
        ...
        boolean connect(Sq s1) {
            ...
            int groupNo = -1; int tgroup = this.group();
            if (tgroup == 0 && sgroup == 0) {
                groupNo = 0;
            } else if (tgroup == 0) {
                Sq pred = this;
                while (pred._successor != null) {
                    pred.successor()._sequenceNum = pred._sequenceNum + 1;
                    pred = pred._successor;
                }
                releaseGroup(sgroup);
                groupNo = tgroup;
            } else if (sgroup == 0) {
                Sq succ = s1;
                while (succ._predecessor != null) {
                    succ.predecessor()._sequenceNum = succ._sequenceNum - 1;
                    succ = succ.predecessor();
                }
                releaseGroup(tgroup);
                groupNo = sgroup;
            } else {
                groupNo = joinGroups(tgroup, sgroup);
            }
            Sq pred = this;
            while (pred.predecessor() != null) {
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                ...
                if (next.successor() == null) {
                    next._head = next; next._group = -1;
                    if (this.predecessor() == null) {
                        this._group = -1; releaseGroup(tgroup);
                    }
                } else {
                    Sq sq = next; int nwGrp = this.group();
                    if (this.predecessor() == null) {
                        this._group = -1;
                    } else {
                        nwGrp = newGroup();
                    ...
                    while (sq != null) {
                        sq._head = next; sq._group = nwGrp; sq = sq.successor();
                    }
                }
            } else {
                boolean ffixed = false; Sq cr = this;
                while (cr != null && !ffixed) {
                    ffixed = cr.hasFixedNum(); cr = cr.predecessor();
                } if (!ffixed) {
                    Sq f = this.head(); int groupNo = -1;
                    if (this.predecessor() != null) {
                        groupNo = newGroup();
                    }
                    while (f != null) {
                        f._sequenceNum = 0; f._group = groupNo;
                        f = f.successor();
                    }
                }
                boolean bfixed = false; Sq cr2 = next;
                while (cr2 != null && !bfixed) {
                    bfixed = cr2.hasFixedNum(); cr2 = cr2.successor();
                }
                Sq c2 = next;
                if (!bfixed) {
                    next._head = next; int groupNo = -1;
                    if (next.successor() != null) {
                        groupNo = newGroup();
                    }
                    while (c2 != null) {
                        c2._sequenceNum = 0; c2._head = next;
                        c2._group = groupNo; c2 = c2.successor();
                    }
                } else {
                    while (c2 != null) {
                        c2._head = next; c2 = c2._successor;
                    }
                }
            }
        }
        ...
        private int _group;
        /** True if assigned a fixed sequence number. */
