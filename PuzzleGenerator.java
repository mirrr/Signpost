...
class PuzzleGenerator implements PuzzleSource {
     ...
     *  a single possible successor).  Return true if changes were made. */
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        Sq connctablSq = null; Sq correct = null;
        int numOfConnectxn = 0; boolean sequencedNextFound = false;
        if (start.successors() != null) {
            for (Place succ: start.successors()) {
                Sq next = model.get(succ.x, succ.y);
                if (start.connectable(next)) {
                    if (next.sequenceNum() - 1 == start.sequenceNum()) {
                        sequencedNextFound = true;
                        correct = next;
                    } else {
                        boolean a = start.sequenceNum() == 0;
                        boolean b = next.predecessor() == null;
                        boolean c = start.sequenceNum() > 0;
                        boolean d = next.sequenceNum() == 0;
                        if ((a && b) || (c && d)) {
                            numOfConnectxn++;
                            connctablSq = next;
                        }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        Sq connctablSq = null; int numOfConnections = 0;
        for (Place pred: end.predecessors()) {
            Sq prior = model.get(pred.x, pred.y);
            if (prior.connectable(end)) {
                boolean a = prior.successor() == null && end.sequenceNum() == 0;
                if (a || (end.sequenceNum() > 0 && prior.sequenceNum() == 0)) {
                    numOfConnections++;
                    connctablSq = prior;
                }
            }
